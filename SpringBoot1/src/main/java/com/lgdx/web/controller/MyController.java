...
public class MyController {
	...
	public String join(MsgMember mem, Model model) {
		...
		// 2. DB에 연결해서 데이터 추가
		mapper.join(mem);
		// * model 영역에 회원가입한 id만 담아서 다음페이지로 넘겨주기
	...
	public String login(MsgMember mem, HttpSession session) {
		...
		// main페이지로 이동 시, 해당 회원에 대한 정보를 담아서 이동할 수 있도록 한다!
	...
	// 업데이트 기능을 위한 요청 진행하기
	// 1. 요청 Mapping
	// 2. 응답의 구조 생성 (form 태그를 통해 입력받은 데이터 활용!)
	// 3. DB에 해당 데이터를 전달 -> update sql 실행!
	// --> email이 일치할 때 pw, tel, address 수정되도록
	// 4. 결과 화면은 main으로 이동
	@PostMapping("/update")
...
//		mem.setEmail(mem2.getEmail());
	...
	public String delete(@RequestParam("email") String email) {
		...
		// 쿼리 파라미터 중 email 값을 String email 변수에 바인딩
		mapper.delete(email);
